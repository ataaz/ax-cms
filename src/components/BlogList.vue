...
.new-post-btn:active { transform: scale(0.98); }
...
.stat-num { font-family: var(--font-mono); font-size: 1.4rem; color: var(--text); line-height: 1; }
.stat-num.pub { color: var(--green); }
.stat-label { font-size: 0.72rem; color: var(--text-3); text-transform: uppercase; letter-spacing: 0.08em; font-family: var(--font-mono); }
...
.cat-pill.active { background: rgba(201,120,74,0.12); border-color: rgba(201,120,74,0.35); color: var(--accent-h); }

.cat-count { font-family: var(--font-mono); font-size: 0.68rem; background: var(--surface-2); border-radius: 999px; padding: 0.05rem 0.4rem; color: var(--text-3); }
.cat-pill.active .cat-count { background: rgba(201,120,74,0.2); color: var(--accent); }

.loading-state { margin-top: 1rem; }
.loading-rows { display: flex; flex-direction: column; gap: 0.75rem; }
.skeleton-row { height: 88px; background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); animation: pulse 1.6s ease-in-out infinite both; }
@keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.4; } }

.error-state { display: flex; align-items: center; gap: 0.75rem; padding: 1rem 1.25rem; background: var(--red-bg); border: 1px solid rgba(194,80,80,0.25); border-radius: var(--radius); color: #d98080; font-size: 0.875rem; }
.error-icon { font-size: 1rem; flex-shrink: 0; }

.empty-state { text-align: center; padding: 5rem 2rem; animation: fadeUp 0.5s ease both; }
.empty-glyph { font-size: 2rem; color: var(--border-2); margin-bottom: 1rem; }
.empty-title { font-family: var(--font-serif); font-size: 1.5rem; color: var(--text-2); margin-bottom: 0.4rem; }
...
.post-row:hover { border-color: var(--border-2); background: var(--surface-2); }

.post-left { flex: 1; min-width: 0; }

.post-title-row { display: flex; align-items: center; gap: 0.65rem; margin-bottom: 0.45rem; }

.status-dot { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }
.status-dot.pub { background: var(--green); box-shadow: 0 0 5px var(--green); }
.status-dot.draft { background: var(--text-3); }

.post-title { font-family: var(--font-serif); font-size: 1.15rem; font-weight: 500; color: var(--text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; line-height: 1.3; }

.post-preview { font-size: 0.82rem; color: var(--text-2); line-height: 1.55; margin-bottom: 0.6rem; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }

.post-meta { display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap; }

.badge { font-family: var(--font-mono); font-size: 0.67rem; font-weight: 500; padding: 0.15rem 0.55rem; border-radius: 3px; text-transform: uppercase; letter-spacing: 0.06em; }
...
.meta-category { font-size: 0.75rem; color: var(--accent); font-family: var(--font-mono); }
.meta-date { font-family: var(--font-mono); font-size: 0.75rem; color: var(--text-3); }

.post-tags { display: flex; flex-wrap: wrap; gap: 0.3rem; margin-top: 0.5rem; }
.post-tag { font-family: var(--font-mono); font-size: 0.67rem; padding: 0.1rem 0.45rem; background: var(--surface-2); border: 1px solid var(--border-2); border-radius: 3px; color: var(--text-3); }
.post-tag-more { font-family: var(--font-mono); font-size: 0.67rem; color: var(--text-3); padding: 0.1rem 0.3rem; }

.post-actions { display: flex; flex-direction: column; gap: 0.35rem; flex-shrink: 0; }

.action-btn { padding: 0.3rem 0.85rem; border-radius: 4px; border: 1px solid transparent; font-size: 0.78rem; font-weight: 500; cursor: pointer; transition: all 0.15s; letter-spacing: 0.02em; min-width: 80px; text-align: center; font-family: var(--font-sans); }
...
.action-btn.toggle:hover:not(:disabled) { border-color: var(--green); color: var(--green); background: var(--green-bg); }

.action-btn.delete { background: transparent; border-color: transparent; color: var(--text-3); }
.action-btn.delete:hover:not(:disabled) { border-color: rgba(194,80,80,0.3); color: var(--red); background: var(--red-bg); }
</style>
